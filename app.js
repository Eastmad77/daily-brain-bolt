// ===== Brain ⚡ Bolt — App.js v3.14.2 (MATCH decoys added, safe pass) =====
const CSV_URL =
...
// ✅ NEW: decoys on right column (high challenge)
// Keeps layout the same; only increases right-side tiles.
const MATCH_DECOYS = 6;
...
if (!qBox || !choicesDiv) {
    ...
    // ✅ NEW: build decoy pool from Round 2's 12 questions (options + answers),
    // excluding the 6 correct answers used in the pairs.
    function buildMatchDecoys(qs12, correctAnswers, maxDecoys) {
        const pool = [];
        const seen = new Set();

        const correctSet = new Set((correctAnswers || []).map((t) => norm(t)));
        ...
        const push = (v) => {
            ...
            const key = norm(s);
            if (!key) return;
            if (correctSet.has(key)) return; // never allow a decoy to equal any correct answer tile
        ...
        (qs12 || []).forEach((q) => {
            // Add all option texts
            push(q?.OptionA ?? q?.optionA ?? q?.optiona);
            push(q?.OptionB ?? q?.optionB ?? q?.optionb);
            push(q?.OptionC ?? q?.optionC ?? q?.optionc);
            push(q?.OptionD ?? q?.optionD ?? q?.optiond);

            // Also consider Answer text (in case Answer is not exactly one of the options)
            push(resolveCorrectText(q));
        });

        shuffleArray(pool);
        return pool.slice(0, Math.max(0, maxDecoys || 0));
    }
    ...
    function startMatchRound() {
        ...
        // ✅ NEW: generate decoys for right side
        const correctAnswers = pairs.map((p) => p.right);
        const decoys = buildMatchDecoys(roundQuestions, correctAnswers, MATCH_DECOYS);
        ...
        matchState = {
            ...
            // ✅ UPDATED: right tiles = correct answers + decoys (decoys never match any left tile)
            rightTiles: shuffleArray([
                ...pairs.map((p) => ({ side: "R", pairId: p.pairId, text: p.right })),
                ...decoys.map((t, i) => ({ side: "R", pairId: `d${i}`, text: t })),
            ]),

            locked: false,
            wrapEl: null, // match grid wrapper for connection line
        };
