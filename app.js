// ===== Brain ⚡ Bolt — App.js v3.10.1 (splash robust) =====
// Fixes:
// - Success splash buttons clickable (aria-hidden removed when showing)
// - Startup splash now hides on DOMContentLoaded, on window load, AND via fallback timeout
...
// Hide splash as soon as DOM is interactive (no image wait)
document.addEventListener('DOMContentLoaded', () => setTimeout(killStartSplash, 900));

// Also hide on full load (in case you want a bit longer animation)
window.addEventListener('load', () => setTimeout(killStartSplash, 900));

// Hard fallback: if neither event fires (blocked resource), still remove splash
