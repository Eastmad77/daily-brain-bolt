...
// ✅ NEW: Decoy tuning (no layout change)
const MATCH_DECOYS_MAX = 6;     // adds up to 6 extra right tiles (total right tiles = 6 + decoys)
...
if (!qBox || !choicesDiv) {
  ...
  // ✅ NEW: safe truncation for decoys (does not affect correct answers)
  ...
  function collectOptionPool(qs12) {
    const pool = [];
    const safe = (qs12 || []).filter(Boolean);
    for (const q of safe) {
      const Q = (k) => q[k] ?? q[k?.toLowerCase?.()] ?? q[k?.toUpperCase?.()];
      const opts = ["OptionA", "OptionB", "OptionC", "OptionD"].map((k) => asText(Q(k))).filter(Boolean);
      pool hookup: pool.push(...opts);
    }
    return pool;
  }

  // ✅ NEW: builds match pairs + decoy right tiles (right side has 6 + decoys)
  function buildMatchSetFromRoundQuestions(qs12) {
    const safe = (qs12 || []).filter(Boolean);
    ...
    if (pairs.length < MATCH_PAIRS) return { pairs, leftTiles: [], rightTiles: [] };

    // Collect option pool from all 12 questions (for decoys)
    const Q = (k) => (row) => row?.[k] ?? row?.[k?.toLowerCase?.()] ?? row?.[k?.toUpperCase?.()];
    const optPoolRaw = [];
    for (const row of safe) {
      const get = (key) => row[key] ?? row[key?.toLowerCase?.()] ?? row[key?.toUpperCase?.()];
      const opts = ["OptionA", "OptionB", "OptionC", "OptionD"].map((k) => asText(get(k))).filter(Boolean);
      optPoolRaw.push(...opts);
    }

    const correctAnswers = pairs.map((p) => p.right);
    const correctSet = new Set(correctAnswers.map((s) => norm(s)));

    // Make a clean decoy pool: unique, non-empty, not equal to any correct answer, reasonable length
    let decoys = uniqByNorm(optPoolRaw)
      .filter((s) => !correctSet.has(norm(s)))
      .filter((s) => {
        const len = asText(s).length;
        return len >= 2 && len <= 120;
      })
      .map((s) => clampText(s, 60));

    // If decoy pool is tiny, allow some correct answers as decoys is BAD — so instead just run fewer decoys.
    decoys = shuffleArray(decoys.slice());

    const decoyCount = Math.min(MATCH_DECOYS_MAX, decoys.length);
    const decoyTiles = decoys.slice(0, decoyCount).map((t, i) => ({
      side: "R",
      // decoys never match: unique pairId that won't equal any left pairId
      pairId: `d${i}`,
      text: t,
      isDecoy: true,
    }));

    // Left tiles always real questions
    const leftTiles = shuffleArray(pairs.map((p) => ({ side: "L", pairId: p.pairId, text: p.left })));

    // Right tiles: ALL correct answers + decoys, shuffled
    const rightTiles = shuffleArray([
      ...pairs.map((p) => ({ side: "R", pairId: p.pairId, text: p.right, isDecoy: false })),
      ...decoyTiles,
    ]);

    return { pairs, leftTiles, rightTiles };
  }

  function startMatchRound() {
    resetQuestionTimerUI();

    const built = buildMatchSetFromRoundQuestions(roundQuestions);
    const pairs = built.pairs;

    if (!pairs || pairs.length < MATCH_PAIRS) {
  ...
  function makeMatchButton(tile) {
    ...
    if (tile.isDecoy) b.dataset.decoy = "1";
  ...
  function onMatchTap(btn) {
    ...
    if (isCorrect) {
      ...
      // end when all real pairs solved, or if you’ve used up attempts
